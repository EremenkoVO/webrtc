...
try {
  ...
  console.error('[Main] ⚠ Native audio capture module not available:', (error as Error).message);
...
app.whenReady().then(() => {
  ...
  const startAudioPoll = () => {
    const poll = async () => {
      if (!audioCapturePolling || !audioCapture) return;
      try {
        ...
        if (audioCapturePolling) setTimeout(poll, 10);
      } catch (error) {
        console.error('[AudioCapture] Poll error:', error);
        const win = BrowserWindow.getAllWindows()[0];
        if (win && !win.isDestroyed())
          win.webContents.send('audio-capture-error', (error as Error).message);
  ...
  ipcMain.handle('get-audio-applications', async () => {
    try {
      ...
      return apps.map((app: { pid: number; name: string; icon?: string }) => ({
        pid: Number(app.pid),
        name: app.name,
  ...
  // Use system picker to let user choose which screen/window to share
      ...
      try {
        ...
        const filteredSources = sources.filter((source) => {
          ...
          if (
            ...
            const displaySurface = (
              videoRequest as { displaySurface?: string }
            ).displaySurface;
        ...
        const handler = (event: Electron.IpcMainEvent, sourceId: string | null, includeAudio: boolean) => {
          ...
          ipcMain.removeListener(handlerId, handler);
          handleSourceSelection(event, sourceId, includeAudio);
        };
        
        ipcMain.on(handlerId, handler);
    ...
    window.addEventListener('beforeunload', (e) => {
      if (!sourceSelected) {
        ...
        selectionWindow.once('closed', () => {
          ...
          setTimeout(() => {
            ipcMain.removeAllListeners(handlerId);
