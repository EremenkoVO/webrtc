...
export const useChatStore = defineStore('chat', () => {
  ...
  function connect(roomId: string, userName: string) {
    ...
    if (ws.value && ws.value.readyState === WebSocket.OPEN && currentRoomId.value === roomId && username.value === userName) {
      return
    }
    
    // Если подключены к другой комнате, отключаемся сначала
    if (ws.value && currentRoomId.value !== roomId) {
      typingUsers.value.clear()
      disconnect()
      // Ждем завершения отключения перед новым подключением
      setTimeout(() => {
        if (!ws.value) {
          connectionState.value = 'connecting'
          currentRoomId.value = roomId
          username.value = userName
          createWebSocketConnection(roomId, userName)
        }
      }, 100)
      return
    }

    // Если уже идет подключение к той же комнате, не запускаем еще одно
    if (connectionState.value === 'connecting' && currentRoomId.value === roomId) {
      return
    }
  ...
  function createWebSocketConnection(roomId: string, userName: string) {
    ...
    try {
      ...
      ws.value.onclose = (event) => {
        const wasConnectedToRoom = currentRoomId.value === roomId
        connectionState.value = 'disconnected'
        ws.value = null
        // Переподключаемся только если это не было намеренное отключение и мы все еще в той же комнате
        if (wasConnectedToRoom && currentRoomId.value === roomId && reconnectAttempts.value < maxReconnectAttempts && event.code !== 1000) {
          attemptReconnect()
        }
      }
    } catch (error) {
  ...
  function attemptReconnect() {
    ...
    // Если уже идет переподключение, не запускаем еще одно
    if (connectionState.value === 'reconnecting' && reconnectTimeout.value) {
      return
    }
    ...
    reconnectTimeout.value = setTimeout(() => {
      // Проверяем, что мы все еще хотим подключиться к той же комнате
      if (currentRoomId.value && username.value && !ws.value) {
        createWebSocketConnection(currentRoomId.value, username.value)
      } else {
        reconnectAttempts.value = 0
      }
    }, delay)
  ...
  function disconnect() {
    ...
    reconnectAttempts.value = 0 // Сбрасываем счетчик попыток при явном отключении
    if (ws.value) {
      // Используем код 1000 (нормальное закрытие) чтобы предотвратить автоматическое переподключение
      ws.value.close(1000)
      ws.value = null
    }
    connectionState.value = 'disconnected'
