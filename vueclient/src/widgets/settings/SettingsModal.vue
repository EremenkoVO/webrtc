...
watch(() => settings.notificationsEnabled, (v) => {
  ...
  <Teleport to="body">
    <Transition name="modal">
      ...
      >
        ...
        >
      <!-- Header -->
      <div class="px-6 py-4 border-b border-dc-separator/40 flex items-center justify-between flex-shrink-0">
      ...
      <div class="px-6 py-4 space-y-6 overflow-y-auto dc-scrollbar-thin">
        ...
        <div class="flex items-start gap-3">
          <input
            ...
            @change="(e) => settings.setNotificationsEnabled((e.target as HTMLInputElement).checked)"
          />
          <label :for="'settings-notifications'" class="flex-1 cursor-pointer">
            <span class="block text-sm font-medium text-dc-text-heading">{{ t('settings.notifications') }}</span>
            <span class="block text-xs text-dc-text-muted mt-0.5">{{ t('settings.notificationsDesc') }}</span>
          </label>
        </div>
        ...
        <div class="flex items-start gap-3">
          ...
          <label :for="'settings-sound-connect'" class="flex-1 cursor-pointer">
            <span class="block text-sm font-medium text-dc-text-heading">{{ t('settings.soundOnConnect') }}</span>
            <span class="block text-xs text-dc-text-muted mt-0.5">{{ t('settings.soundOnConnectDesc') }}</span>
          </label>
        </div>
      </div>
